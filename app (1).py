...
<style>
  ...
  /* Hide deploy button and top-right menu but KEEP the header so the
     mobile sidebar hamburger toggle remains accessible */
  #MainMenu { visibility: hidden; }
  footer { visibility: hidden; }
  /* Hide only the decoration/deploy items, not the whole header */
  header [data-testid="stToolbar"],
  header [data-testid="stDecoration"],
  .stDeployButton { display: none !important; }
  /* On desktop collapse the header height to zero so it doesn't take space */
  @media (min-width: 769px) {
    header[data-testid="stHeader"] { height: 0 !important; min-height: 0 !important; overflow: hidden; }
  }
  /* On mobile keep the header visible at its natural height for the hamburger */
  ...
  /* ---- KPI Cards ---- */
  /* Root is <section> (not <div>) to prevent Streamlit's fragment parser from
     leaking a bare </div> into the rendered output — see kpi_card() helper. */
  section.kpi-card {
  ...
  section.kpi-card:hover { box-shadow: var(--shadow-md); transform: translateY(-2px); }
  section.kpi-card::before {
    content: '';
    position: absolute; top: 0; left: 0; right: 0;
    height: 3px;
  }
  section.kpi-card.blue::before  { background: var(--primary); }
  section.kpi-card.green::before { background: var(--success); }
  section.kpi-card.amber::before { background: var(--warning); }
  section.kpi-card.red::before   { background: var(--danger); }
  section.kpi-card.purple::before{ background: var(--purple); }
  ...
  .sidebar-nav-item.active  { background: #4F46E5 !important; color: #ffffff !important; font-weight: 600; }

  /* Active nav button — Streamlit renders buttons with data-testid="baseButton-secondary"
     inside a div whose key attribute is surfaced as the button's parent element id.
     We target every sidebar button first (idle state) then override the active one
     via a dynamically injected <style> block written from Python. */
  ...
  /* Active page button — class injected dynamically via st.markdown below */
  ...
  section.kpi-card, .card, .partner-card { animation: fadeUp .3s ease both; }
...
if st.session_state.dark_mode:
    ...
    .card, section.kpi-card, .partner-card { background: #1E293B !important; border-color: #334155 !important; }
...
def kpi_card(label, value, icon, color="blue", change=None, change_dir="up"):
    if change:
        ...
        change_html = (
            f'<span class="kpi-change {change_dir}">{arrow} {change}</span>'
        )
    else:
        change_html = ""
    # Use a <section> root (not <div>) so Streamlit's markdown sanitiser
    # doesn't try to balance/close it against surrounding fragment <div>s,
    # which was causing the literal </div> leak.
    return (
        f'<section class="kpi-card {color}">'
        f'<div class="kpi-icon {color}">{icon}</div>'
...
def sidebar():
    with st.sidebar:
        ...
        # Inject a targeted style for whichever nav button is active.
        # Streamlit wraps each st.button in a <div data-testid="stButton"> whose
        # closest uniquely-identifiable ancestor carries the widget key as part of
        # an auto-generated element id.  The most reliable cross-version selector
        # is to match on the button's visible text content via :has() + contains,
        # but broad browser support requires a simpler approach: we write one rule
        # per active page that matches the button by its label text using the
        # adjacent sibling / nth-of-type trick isn't available, so instead we
        # target the key-derived id Streamlit places on the outermost div wrapper.
        active_page = st.session_state.page
        active_styles = ""
        for _, label in nav_items:
            key = f"nav_{label}"
            if label == active_page:
                # Streamlit ≥1.30 sets data-testid on the button wrapper div and
                # exposes the key via the element's id in the format:
                #   <div id="..." data-testid="element-container"> ... <button>
                # The most stable selector available is matching the button that
                # contains the label text – we use the :has selector with a
                # generated class on a wrapping element, but the simplest approach
                # that works reliably is to scope by key via the Streamlit
                # generated id prefix.
                active_styles += f"""
                    section[data-testid="stSidebar"]
                    div[data-testid="stButton"]:has(button[key="{key}"]) button,
                    section[data-testid="stSidebar"]
                    div[data-testid="stButton"] button[data-testid="{key}"] {{
                        background: #4F46E5 !important;
                        color: #ffffff !important;
                        font-weight: 600 !important;
                    }}
        ...
        # Fallback: use nth-of-type positional targeting as the most reliable
        # cross-version approach. We find the 1-based index of the active item.
        active_idx = next((i + 1 for i, (_, lbl) in enumerate(nav_items) if lbl == active_page), None)
        if active_idx is not None:
            active_styles += f"""
                section[data-testid="stSidebar"]
                div[data-testid="stVerticalBlock"]
                > div:nth-of-type({active_idx + 2})
                .stButton > button {{
                    background: #4F46E5 !important;
                    color: #ffffff !important;
                    font-weight: 600 !important;
