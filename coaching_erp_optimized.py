...
<style>
  ...
  /* ---- Header — keep Streamlit header alive for hamburger ---- */
  ...
  /* Always show the sidebar collapse/hamburger control */
  ...
  /* Selected item — works in modern browsers */
  ...
  /* Hide radio circle and widget label */
  ...
  section.kpi-card.purple::before { background: var(--teal); }
  ...
  .kpi-icon.purple { background: var(--teal-l); }
  ...
  /* Sidebar utility buttons (dark mode, refresh, logout) */
  ...
  .wa-btn:hover { background: #16A34A; transform: translateY(-1px); color: white; }
  ...
  /* Ensure login inputs are always visible regardless of Streamlit theme */
  .login-wrap .stTextInput input {
    background: white !important;
    color: #0F172A !important;
    border: 1.5px solid #E2E8F0 !important;
  }
  .login-wrap .stTextInput input::placeholder { color: #94A3B8 !important; }
  .login-wrap label {
    color: #0F172A !important;
    font-weight: 600 !important;
  }
...
if st.session_state.dark_mode:
    ...
    .kpi-label, .partner-label, .page-subtitle, .sidebar-user-role { color: #94A3B8 !important; }
...
def kpi_card(label, value, icon, color="blue", change=None, change_dir="up"):
    if change:
        ...
        change_html = f'<span class="kpi-change {change_dir}">{arrow} {change}</span>'
    else:
        change_html = ""
    return (
...
PALETTE = ['#059669', '#10B981', '#F59E0B', '#EF4444', '#0D9488', '#06B6D4']
...
# Map display label → page key
NAV_LABELS = {item: item.split("  ", 1)[1] for item in NAV_ITEMS}
...
def sidebar():
    with st.sidebar:
        ...
        current_index = next(
            (i for i, item in enumerate(NAV_ITEMS) if NAV_LABELS[item] == current_page),
            0
        )

        # Radio navigation — no CSS hacks needed; Streamlit handles active state
        selected = st.radio(
            "Navigation",
            NAV_ITEMS,
            index=current_index,
        ...
        # Utility controls
        col_dm, col_ref = st.columns(2)
        with col_dm:
            dm_label = "☀️ Light" if st.session_state.dark_mode else "🌙 Dark"
            if st.button(dm_label, key="darkmode_toggle", use_container_width=True):
...
def page_dashboard():
    ...
    if not pdf.empty or not edf.empty:
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=trend['date'], y=trend['Income'],   name='Income',   line=dict(color=PALETTE[0], width=2.5), fill='tozeroy', fillcolor='rgba(5,150,105,.08)'))
        fig.add_trace(go.Scatter(x=trend['date'], y=trend['Expenses'], name='Expenses', line=dict(color=PALETTE[2], width=2.5)))
        fig.add_trace(go.Scatter(x=trend['date'], y=trend['Profit'],   name='Profit',   line=dict(color=PALETTE[1], width=2, dash='dot')))
...
def page_analytics():
    ...
    if not pdf.empty or not edf.empty:
        ...
        trend['Profit'] = trend['Income'] - trend['Expenses']
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=trend['date'], y=trend['Income'],   name='Income',   line=dict(color=PALETTE[0], width=2.5), fill='tozeroy', fillcolor='rgba(5,150,105,.1)'))
        ...
        fig.add_trace(go.Bar(    x=trend['date'], y=trend['Profit'],   name='Profit',   marker_color=[PALETTE[0] if v >= 0 else PALETTE[3] for v in trend['Profit']], opacity=0.6))
        st.plotly_chart(plotly_defaults(fig), use_container_width=True)
    ...
    if not sdf.empty:
        ...
        fig = px.bar(bar_df, x='Status', y='Amount', color='Status',
                     color_discrete_map={'Collected': PALETTE[0], 'Pending': PALETTE[3]},
                     labels={'Amount': '₹'})
        fig.update_layout(showlegend=False)
        st.plotly_chart(plotly_defaults(fig), use_container_width=True)
